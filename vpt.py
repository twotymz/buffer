...
class _Node:
    ...
    def help_find(self, item, distances, heap, distance):
        ...
        for child in self.children:
            heapq.heappush(heap, (child.minimum_distance(
                new_distances), 1, child, new_distances))
...
def _make_node(items, distance, max_children):
    ...
    items = [(item[0], item[1] + (distance(node.vantage, item[0]),))
             for item in items]
    ...
    for i in xrange(n_children):
        split_points.append(
            distance_list[(i + 1) * (len(distance_list) - 1) // n_children])

    for i in xrange(n_children):
        child_items = [item for item in items if split_points[i]
                       < item[1][-1] <= split_points[i + 1]]
...
class VP_tree:
    def __init__(self, items, distance, max_children=2):
        ...
        self.root = _make_node(self.items, distance, max_children)
